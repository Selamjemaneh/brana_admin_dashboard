...
import { updateDictionary } from '@/app/state/slice/dictionarySlice'
...
export function EditDictionaryDialog({
    dictionary,
    isOpen,
    onClose,
}: EditDictionaryDialogProps) {
    const dispatch = useAppDispatch()
    const { updating: isSaving } = useAppSelector((state) => state.dictionary)
    ...
    useEffect(() => {
        if (dictionary) {
            setFormData({
                ...
                language_code: dictionary.language_code || 'AM',
                is_premium: !!dictionary.is_premium,
            })
            setImagePreview(getS3PublicUrl(dictionary.cover_image) || null)
            setCoverImage(null)
        }
    }, [dictionary])
    ...
    const handleSave = async () => {
        ...
        formDataPayload.append('is_premium', String(formData.is_premium))
        ...
        const resultAction = await dispatch(updateDictionary({
            id: dictionary._id,
            data: formDataPayload
        }))

        if (updateDictionary.fulfilled.match(resultAction)) {
            toast.success("Dictionary updated successfully", { position: "top-center" })
            onClose()
        } else {
            const errorMsg = (resultAction.payload as any) || 'Failed to update'
            toast.error(typeof errorMsg === 'string' ? errorMsg : "Dictionary has not been updated", { position: "top-center" })
        }
    }
