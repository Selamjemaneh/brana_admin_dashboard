...
import { mockCommentaries, languageCodes } from '@/lib/mock-data'
...
interface CommentaryEditPageProps {
  params: Promise<{
    id: string
  }>
}

export default async function CommentaryEditPage({ params }: CommentaryEditPageProps) {
  ...
  return <CommentaryEditForm commentary={commentary} />
}

function CommentaryEditForm({ commentary }: any) {
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      ...
      router.push(`/admin/commentaries/${commentary._id}`)
    } catch (error) {
      toast({
        title: 'Error',
        description: 'Failed to update commentary',
        variant: 'destructive',
      })
    } finally {
      setIsSaving(false)
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <Card className="border border-border max-w-2xl">
        <CardHeader>
          <CardTitle className="font-serif text-xl sm:text-2xl">Edit Commentary</CardTitle>
          <CardDescription className="text-xs sm:text-sm">
            Update the commentary information and metadata
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            {/* Author */}
            <div className="space-y-2">
              <Label htmlFor="author" className="text-xs sm:text-sm font-medium text-foreground">
                Author
              </Label>
              <Input
                id="author"
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-6">
              ...
              <Link href={`/admin/commentaries/${commentary._id}`} className="w-full sm:w-auto">
