...
interface ViewDictionaryDialogProps {
    dictionary: any
    isOpen: boolean
    onClose: () => void
}

export function ViewDictionaryDialog({
    dictionary,
    isOpen,
    onClose,
}: ViewDictionaryDialogProps) {
    if (!dictionary) return null

    const imageUrl = getS3PublicUrl(dictionary.cover_image_url)

    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle className="font-serif text-2xl">Dictionary Details</DialogTitle>
                </DialogHeader>
                ...
                <div className="grid gap-6 py-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                        <div className="space-y-1">
                            ...
                            <p className="text-sm font-semibold text-foreground">{dictionary.name}</p>
                        </div>
                        <div className="space-y-1">
                            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Language</p>
                            <Badge variant="secondary" className="font-mono">{dictionary.language_code}</Badge>
                        </div>
                        <div className="space-y-1">
                        ...
                        <div className="space-y-1">
                            ...
                            <p className="text-sm text-foreground">{formatDate(new Date(dictionary.created_at))}</p>
                        </div>
                    </div>
                </div>
            </DialogContent>
